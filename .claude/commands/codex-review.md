...
Review = `$RUN.review` (also echoed to stdout). Both lost → fall back to rollout JSONL `~/.codex/sessions/<date>/`, but disambiguate by cwd NOT mtime: rollout's 1st line = `session_meta` w/ `payload.cwd` + `payload.id` (also in filename) → newest rollout where `payload.cwd` == `$PWD` → read its last assistant message.
